...
- **多请求格式**（自动检测）：Anthropic `/v1/messages`、OpenAI Chat Completions、OpenAI Responses API；另有 `compressText` 直接压缩单条字符串（如工具输出原文）
...
- **有损可恢复**：有损压缩的原文写入 CCR store（落盘持久化），输出留 `<<ccr:HASH>>` 标记，可按 key 取回原文
...
// 或直接压缩单条工具输出原文
const r = compressText(toolOutput);

// 有损压缩的原文可按标记取回
const original = retrieve(r.ccrKey!);
```
