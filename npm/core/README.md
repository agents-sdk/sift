...
CSV-schema writes the row count and typed columns once—such as `[200]{id:int,status:string}`—then emits ordinary CSV rows. Heterogeneous arrays can use `__buckets:type` with one schema per category. Nested object arrays and JSON encoded inside strings recursively become compact table objects instead of carrying repeated keys and escaping. Long opaque string, HTML, and base64 cells become recoverable stash markers.
