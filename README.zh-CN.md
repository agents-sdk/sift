...
### **上下文减少 60.6%，估算节省 10,548 tokens，有损基准样例全部恢复成功。**

8 个内置基准场景合计从 **58,017 B 降至 22,859 B**，其中 **4/4 个有损样例均成功恢复完整原文**。[查看完整实测数据。](#实际能节省多少)
...
- **降低上下文成本**：内置基准样例体积减少 **60.6%**，从 58,017 字节降至 22,859 字节。
...
以下数据使用仓库内 8 个固定的 [demo 输入](npm/core/demo/cases)，由已发布的 `0.0.1-alpha.7` 包实测。测试口径和复现方式见 [BENCHMARK.md](BENCHMARK.md)。
...
| 搜索结果 | 10,057 B | 2,863 B | 71.5% | 2,159 | PASS |
| Git diff | 8,201 B | 8,201 B | 0% | 0 | 原样返回 |
| 混合命令输出 | 9,240 B | 1,601 B | 82.7% | 2,291 | PASS |
| Rust 源代码 | 2,282 B | 350 B | 84.7% | 580 | PASS |
| 独有内容纯文本 | 3,125 B | 3,125 B | 0% | 0 | 原样返回 |
| **合计** | **58,017 B** | **22,859 B** | **60.6%** | **10,548** | **4/4 有损样例恢复成功** |

这是公开样例的透明结果，不代表所有工作负载。表格特意保留了两个未压缩样例：sift 只有在结果更小时才会发布压缩内容，而且保守的纯文本策略不会删除独有事实。`tokensSaved` 使用 sift 内置估算器；实际 token 数和节省比例会随模型 tokenizer 与输入数据变化。
...
对于源码、日志、搜索结果、diff 和整行纯文本，省略提示可以直接指向 stash 文件和准确行范围：
...
| 源代码 | 保留签名和结构，折叠适合省略的函数体；支持 Python、JavaScript、TypeScript、Go、Rust、Java、C、C++ |
| 纯文本 | 只折叠同章节中的完全重复块，独有事实仍然可见 |
