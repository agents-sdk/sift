...
### **上下文减少 73.1%，估算节省 16,795 tokens，有损基准样例全部恢复成功。**

10 个内置基准场景合计从 **76,582 B 降至 20,594 B**，其中 **8/8 个有损样例均成功恢复完整原文**。[查看完整实测数据。](#实际能节省多少)
...
- **降低上下文成本**：内置基准样例体积减少 **73.1%**，从 76,582 字节降至 20,594 字节。
...
以下数据使用仓库内 10 个固定的 [demo 输入](npm/core/demo/cases)，由当前源码实测（包版本字段为 `0.0.1-alpha.7`）。测试口径和复现方式见 [BENCHMARK.md](BENCHMARK.md)。
...
| **合计** | **76,582 B** | **20,594 B** | **73.1%** | **16,795** | **8/8 有损样例恢复成功** |
