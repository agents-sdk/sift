...
### **上下文减少 52.6%，估算节省 14,016 tokens，有损基准样例全部恢复成功。**

12 个内置基准场景合计从 **88,759 B 降至 42,037 B**，其中 **8/8 个有损样例均成功恢复完整原文**。[查看完整实测数据。](#实际能节省多少)
...
- **降低上下文成本**：内置基准样例体积减少 **52.6%**，从 88,759 字节降至 42,037 字节。
...
以下数据使用仓库内 12 个固定的 [demo 输入](npm/core/demo/cases)，由当前源码实测（包版本字段为 `0.0.1-alpha.7`）。测试口径和复现方式见 [BENCHMARK.md](BENCHMARK.md)。

| 场景 | 输入 | 输出 | 体积减少 | 估算节省 token | 恢复验证 |
| --- | ---: | ---: | ---: | ---: | --- |
| JSON 数组 | 18,397 B | 12,448 B | 32.3% | 1,785 | 无损 |
| Pretty JSON | 3,642 B | 973 B | 73.3% | 801 | 无损 |
| 构建日志 | 3,073 B | 1,543 B | 49.8% | 459 | 无损 |
| 搜索结果 | 10,057 B | 3,227 B | 67.9% | 2,049 | PASS |
| Git diff | 23,007 B | 7,795 B | 66.1% | 4,564 | PASS |
| 混合命令输出 | 9,240 B | 3,879 B | 58.0% | 1,608 | 无损 |
...
| Markdown 表格 | 9,479 B | 7,275 B | 23.3% | 661 | PASS |
| **合计** | **88,759 B** | **42,037 B** | **52.6%** | **14,016** | **8/8 有损样例恢复成功** |
