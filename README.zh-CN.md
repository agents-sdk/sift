...
### **上下文减少 71.3%，估算节省 16,161 tokens，有损基准样例全部恢复成功。**

9 个内置基准场景合计从 **75,546 B 降至 21,672 B**，其中 **6/6 个有损样例均成功恢复完整原文**。[查看完整实测数据。](#实际能节省多少)
...
- **降低上下文成本**：内置基准样例体积减少 **71.3%**，从 75,546 字节降至 21,672 字节。
...
以下数据使用仓库内 9 个固定的 [demo 输入](npm/core/demo/cases)，由当前源码实测（包版本字段为 `0.0.1-alpha.7`）。测试口径和复现方式见 [BENCHMARK.md](BENCHMARK.md)。
...
| **合计** | **75,546 B** | **21,672 B** | **71.3%** | **16,161** | **6/6 有损样例恢复成功** |

这是公开样例的透明结果，不代表所有工作负载。表格特意保留了一个未压缩样例：其中疑似凭据触发了全局安全保护。`tokensSaved` 使用 sift 内置估算器；实际 token 数和节省比例会随模型 tokenizer 与输入数据变化。
