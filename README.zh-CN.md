...
### **上下文减少 61.8%，估算节省 13,497 tokens，有损基准样例全部恢复成功。**

8 个内置基准场景合计从 **72,823 B 降至 27,833 B**，其中 **5/5 个有损样例均成功恢复完整原文**。[查看完整实测数据。](#实际能节省多少)
...
- **降低上下文成本**：内置基准样例体积减少 **61.8%**，从 72,823 字节降至 27,833 字节。
...
| **合计** | **72,823 B** | **27,833 B** | **61.8%** | **13,497** | **5/5 有损样例恢复成功** |

这是公开样例的透明结果，不代表所有工作负载。表格特意保留了一个未压缩样例：保守的纯文本策略不会删除独有事实。`tokensSaved` 使用 sift 内置估算器；实际 token 数和节省比例会随模型 tokenizer 与输入数据变化。
