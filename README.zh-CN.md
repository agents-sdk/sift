...
### **上下文减少 62.3%，估算节省 14,129 tokens，有损基准样例全部恢复成功。**

9 个内置基准场景合计从 **75,546 B 降至 28,447 B**，其中 **6/6 个有损样例均成功恢复完整原文**。[查看完整实测数据。](#实际能节省多少)
...
- **降低上下文成本**：内置基准样例体积减少 **62.3%**，从 75,546 字节降至 28,447 字节。
...
以下数据使用仓库内 9 个固定的 [demo 输入](npm/core/demo/cases)，由已发布的 `0.0.1-alpha.7` 包实测。测试口径和复现方式见 [BENCHMARK.md](BENCHMARK.md)。
