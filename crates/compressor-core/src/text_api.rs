...
//! 有损压缩 → token 校验 → CCR 卸载），有损时输出尾部带 `<<ccr:KEY>>` 标记。
...
pub struct TextCompressResult {
    ...
    /// 是否发生了实际压缩。
    pub changed: bool,
    /// 是否有损（原文已写入 CCR store，可用 retrieve(key) 回取）。
    pub lossy: bool,
    /// 有损时的取回 key。
...
mod tests {
    ...
    fn large_json_array_lossy_with_marker() {
        ...
        assert!(r.changed);
        assert!(r.lossy);
        assert!(r.text.contains("<<ccr:"));
        assert!(r.text.len() < raw.len());
        assert!(r.tokens_saved > 0);
        // 原文可回取。
        let key = r.ccr_key.as_ref().unwrap();
