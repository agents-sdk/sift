...
//! 非 span 字节逐字保留。
//!
//! CCR 安全性：本模块只负责定位与替换；replace 回调走的是与整块 JSON
//! 相同的分发路径，CCR 按 hash 取键、与位置无关，故嵌入 span 内产生的
//! `<<ccr:HASH>>` 标记同样可恢复。非 span 字节逐字保留，绝不跨边界丢内容。
...
mod tests {
    ...
    #[test]
    fn ccr_marker_span_left_alone() {
        // 已含 CCR 标记的内容照样传给回调（由调用方决定跳过）；
        // 这里验证回调返回 None 时字节不变
