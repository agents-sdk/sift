...
use crate::ccr::{marker_for, CcrStore};
        ...
        strategy.for_each_candidate(msg, &mut |holder, text_field| {
            ...
            match result {
                ...
                BlockOutcome::Lossy {
                    ...
                    store.put(&ccr_key, text);
                    outcome.ccr_stored += 1;
                    outcome.tokens_saved += tokens_saved as i64;
                    holder[text_field] = Value::String(new_text);
                    outcome.blocks_compressed += 1;
                    outcome.changed = true;
...
/// 有损：原文由调用方（Lossy 分支）写入 CCR store。
...
pub(crate) fn process_block_text(
    ...
    // 无损已缩够（≤ 80% 原体积）则跳过有损压缩，避免不必要的 CCR 卸载。
    let reformat_ratio =
    ...
    if content_type == crate::content::ContentType::PlainText {
        if let Some(compressed) = route_mixed_fallback(&current, ctx) {
            let key = crate::ccr::compute_key(text);
    ...
    // CCR 存保护前的真正原文，保证 retrieve 拿回完整内容。
...
mod tests {
    ...
    fn mixed_bash_output_routes_per_section() {
        ...
        assert!(outcome.ccr_stored >= 1, "混合路由应有损+CCR: {outcome:?}");
        let txt = b["messages"][0]["content"][0]["content"].as_str().unwrap();
        // 文本段保留、JSON 段被压、含取回标记。
        assert!(txt.contains("$ gh api repos/x/y/issues"));
        assert!(txt.contains("<<ccr:"));
        assert!(txt.len() < mixed.len());
        // 原文可回取（含完整 JSON）。
        let key_start = txt.rfind("<<ccr:").unwrap();
        let key = &txt[key_start + 6..txt.len() - 2];
        assert_eq!(store.get(key).unwrap(), mixed);
    }

    #[test]
    fn lossless_reformat_short_circuits_ccr() {
        ...
        assert!(outcome.changed, "outcome={:?}", outcome);
        // 无损路径：不写 CCR store。
        assert_eq!(outcome.ccr_stored, 0, "无损短路不应写 CCR: {outcome:?}");
        let txt = b["messages"][0]["content"][0]["content"].as_str().unwrap();
        // 无损结果不含取回标记，且仍可解析回等价 JSON。
        assert!(!txt.contains("<<ccr:"), "无损路径不应有标记: {txt}");
    ...
    fn compresses_large_json_tool_result() {
        ...
        assert!(outcome.ccr_stored >= 1);
        // 压缩后 text 含取回标记。
        let blocks = b["messages"][2]["content"].as_array().unwrap();
        let txt = blocks[0]["content"].as_str().unwrap();
        assert!(txt.contains("<<ccr:"));
        // 标记可回取原文：从尾部 <<ccr:KEY>> 提取 key 查 store。
        let marker_start = txt.rfind("<<ccr:").unwrap();
        let key = &txt[marker_start + "<<ccr:".len()..txt.len() - 2];
        let restored = store.get(key).expect("原文应可从 CCR store 回取");
        assert_eq!(restored, serde_json::to_string(&rows).unwrap());
    }

    #[test]
    fn ccr_stores_original_with_custom_tags() {
    ...
    fn compresses_responses_function_call_output() {
        ...
        // 原文可回取。
        let marker_start = out.rfind("<<ccr:").unwrap();
        let key = &out[marker_start + "<<ccr:".len()..out.len() - 2];
        assert_eq!(store.get(key).unwrap(), raw);
    }
