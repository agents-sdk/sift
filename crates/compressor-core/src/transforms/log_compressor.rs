...
//! - md5 缓存键 → 复用 [`crate::ccr::compute_key`]（blake3 前 24 hex）；
...
//! 原文经 `apply` 返回给调用方写入 CCR store，实现端到端无损。
...
pub struct LogCompressorConfig {
    ...
    /// 是否输出 CCR 取回标记（需要调用 `compress_with_store` 传入 store）。
    pub enable_ccr: bool,
    pub min_lines_for_ccr: usize,
    /// 触发 CCR 存储的压缩率阈值（压缩后/原始字节数低于该值才卸载）。
...
/// 日志压缩器。有损变换：调用方负责把原文写入 CCR store。
...
impl LogCompressor {
    ...
    /// 压缩（不带 CCR store；需要卸载标记时用 `compress_with_store`）。
...
impl OffloadTransform for LogCompressor {
    ...
    fn estimate_bloat(&self, input: &str) -> f64 {
        ...
        if line_count < self.config.min_lines_for_ccr {
            return 0.0;
        }
    ...
    fn apply(
        ...
        if result.original_line_count < self.config.min_lines_for_ccr
            && result.compressed != input
        {
            return Err(TransformError::Internal("short log changed".into()));
        }
        // 返回 (压缩输出, 原文)：调用方把原文写入 CCR store 完成卸载
...
mod tests {
    ...
    fn ccr_skipped_without_store() {
        ...
        assert_eq!(stats.ccr_skip_reason, Some("no store provided"));
    }
