...
//! 7. 达到阈值时追加 CCR 取回标记（原文由调用方经 `OffloadTransform::apply`
//!    返回值卸载到 store）。
//!
...
//!
//! 与参考实现的差异（受依赖约束）：
//! - 不用 `md5`，cache_key 用 blake3（`crate::ccr::compute_key`）。
...
/// 压缩结果。`compressed` 是格式化输出（可含 CCR 标记）；
...
/// apply 返回 (压缩文本, 原文)，由调用方把原文写入 CCR store。
...
src/main.py-44-context line";
    ...
    fn multi_file_results_are_thinned_with_summaries() {
        let compressor = SearchCompressor::new(SearchCompressorConfig {
            max_matches_per_file: 2,
            max_total_matches: 10,
            min_matches_for_ccr: 100, // 关掉标记便于断言正文
            ..Default::default()
        });
    ...
    fn ccr_marker_emitted_when_thresholds_clear() {
        let compressor = SearchCompressor::new(SearchCompressorConfig {
            ...
            min_compression_ratio_for_ccr: 0.95, // 测试用宽松阈值
            ..Default::default()
        });
        let mut content = String::new();
        for i in 1..=12 {
        ...
        assert!(result
            .compressed
            .contains(&ccr::marker_for(result.cache_key.as_ref().unwrap())));
    }
    ...
    fn transform_metadata_and_bloat() {
        ...
        assert_eq!(key, ccr::compute_key(good));
    }
