...
//! 卸载进 CCR store 后通过 `<<ccr:HASH>>` 标记恢复，端到端无损。
//!
//! # 相对参考实现的保留 / 简化
//!
...
//! - 缓存键复用 [`crate::ccr::compute_key`]（blake3 前 24 hex），不引入 md5。
...
/// TextCrusher 是有损压缩（丢弃低分段落），原文由调用方写入 CCR store，
...
impl OffloadTransform for TextCrusher {
    ...
    /// cache key：与 [`crate::ccr::compute_key`] 统一（blake3 前 24 hex），
...
mod tests {
    ...
    fn estimate_bloat_and_cache_key_deterministic() {
        ...
        assert_eq!(tc.cache_key(&long), ccr::compute_key(&long));
    }

    #[test]
    fn apply_returns_original_for_ccr() {
        let content = doc(40);
        let (compressed, original) = TextCrusher::default()
            .apply(&content, &ctx(Some("topic 7")))
            .expect("长文本应可压缩");
        assert_eq!(original, content, "原文应原样返回供 CCR 卸载");
        assert!(compressed.len() < content.len());
