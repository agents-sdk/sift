...
//!    （原文由调用方经 `OffloadTransform::apply` 卸载到 stash store）。
//!
//! trait 选择：本压缩器**有损**（丢弃 hunk 与上下文行），因此实现
//! [`crate::transforms::OffloadTransform`] 而非 `ReformatTransform`。
//!
//! 与参考实现的差异（受依赖约束）：
//! - 不用 `md5`/`regex`/`tracing`：cache_key 用 blake3（`crate::stash`），
...
pub struct DiffCompressorConfig {
    ...
    /// 参考实现名为 min_lines_for_stash（名不副实地门控整条压缩路径）。
    pub min_lines_for_stash: usize,
    /// stash 标记的压缩比阈值：压缩后行数 < 原始行数 × 此值才追加。
...
impl DiffCompressor {
    ...
    pub fn compress_with_stats(
        ...
        if original_line_count < self.config.min_lines_for_stash {
            ...
            stats.stash_skipped_reason = Some("input below min_lines_for_stash".into());
        ...
        if diff_files.is_empty() {
            stats.output_lines = original_line_count;
            stats.compression_ratio = 1.0;
            stats.stash_skipped_reason = Some("no diff sections parsed".into());
            return (pass_through_result(content, original_line_count), stats);
        }
        ...
        // stash 层：节省达标才追加标记。注意 compressed_line_count 在追加标记
        // **之前**捕获（标记自身的 "compressed to N" 与结果字段都用它），
        // 与参考实现一致。
        let savings_threshold = self.config.min_compression_ratio_for_stash;
        ...
        } else if !self.config.enable_stash {
            stats.stash_skipped_reason = Some("stash disabled".into());
        } else {
            stats.stash_skipped_reason = Some(format!(
...
mod tests {
    ...
    fn short_input_passes_through() {
        ...
        // 4 行 < min_lines_for_stash(50) → 原样返回。
    ...
    fn context_trim_keeps_lines_within_window() {
        ...
        let cfg = DiffCompressorConfig {
            ...
            min_compression_ratio_for_stash: 0.1, // 关掉标记方便断言正文
            ..Default::default()
        };
    ...
    #[test]
    fn min_compression_ratio_for_stash_is_configurable() {
        ...
        assert!(stats.stash_skipped_reason.is_some());
    }
    ...
    fn rename_markers_are_preserved_in_output() {
        ...
        let cfg = DiffCompressorConfig {
            min_lines_for_stash: 5,
            min_compression_ratio_for_stash: 0.1,
            ..Default::default()
        };
        let r = DiffCompressor::new(cfg).compress(input, "");
        assert!(
    ...
    fn transform_metadata_and_bloat() {
        ...
        assert_eq!(key, stash::compute_key(&diff));
    }
