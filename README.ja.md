...
### **コンテキストを 71.3% 削減。推定 18,485 トークンを節約。非可逆ベンチマークはすべて復元成功。**

12 個の内蔵ベンチマーク全体で **86,467 B から 24,846 B** へ削減し、**非可逆 10/10 ケースで原文の復元に成功**しました。[測定結果の詳細を見る。](#どれくらい削減できるか)
...
- **コンテキストコストの削減** — 内蔵ベンチマークでは 86,467 バイトから 24,846 バイトへ、**71.3% 削減**しました。
...
| CSV テーブル | 7,187 B | 2,258 B | 68.6% | 1,479 | PASS |
| **合計** | **86,467 B** | **24,846 B** | **71.3%** | **18,485** | **非可逆 10/10 件を復元** |
...
| CSV、TSV、Markdown テーブル | 列構造を厳密に解析して SmartCrusher に渡し、代表レコードと異常レコードを保持 |
