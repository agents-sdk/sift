...
### **コンテキストを 71.3% 削減。推定 16,161 トークンを節約。非可逆ベンチマークはすべて復元成功。**

9 つの内蔵ベンチマーク全体で **75,546 B から 21,672 B** へ削減し、**非可逆 6/6 ケースで原文の復元に成功**しました。[測定結果の詳細を見る。](#どれくらい削減できるか)
...
- **コンテキストコストの削減** — 内蔵ベンチマークでは 75,546 バイトから 21,672 バイトへ、**71.3% 削減**しました。
...
リポジトリ内の固定された 9 つの [demo 入力](npm/core/demo/cases)を、現在のソースツリー（パッケージバージョン `0.0.1-alpha.7`）で測定した結果です。測定方法と再現手順は [BENCHMARK.md](BENCHMARK.md) を参照してください：

| シナリオ | 入力 | 出力 | サイズ削減 | 推定節約トークン | 復元 |
| --- | ---: | ---: | ---: | ---: | --- |
| JSON 配列 | 18,397 B | 1,888 B | 89.7% | 4,953 | PASS |
...
| 混合コマンド出力 | 9,240 B | 1,037 B | 88.8% | 2,460 | PASS |
| Rust ソースコード | 2,282 B | 402 B | 82.4% | 564 | PASS |
| 繰り返しプレーンテキスト | 2,723 B | 454 B | 83.3% | 680 | PASS |
| 固有情報と機密値の保護 | 3,125 B | 3,125 B | 0% | 0 | 変更なし |
| **合計** | **75,546 B** | **21,672 B** | **71.3%** | **16,161** | **非可逆 6/6 件を復元** |

これは公開 fixture の透明な測定結果であり、すべてのワークロードに対する保証ではありません。変更なしの 1 件は、認証情報らしい値がグローバル安全ゲートにより保護された結果です。`tokensSaved` は sift 内蔵の推定値です。実際のトークン数と削減量はモデルの tokenizer と入力データによって変わります。
