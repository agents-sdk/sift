...
### **コンテキストを 71.5% 削減。推定 17,006 トークンを節約。非可逆ベンチマークはすべて復元成功。**

11 個の内蔵ベンチマーク全体で **79,280 B から 22,588 B** へ削減し、**非可逆 9/9 ケースで原文の復元に成功**しました。[測定結果の詳細を見る。](#どれくらい削減できるか)
...
- **コンテキストコストの削減** — 内蔵ベンチマークでは 79,280 バイトから 22,588 バイトへ、**71.5% 削減**しました。
...
リポジトリ内の固定された 11 個の [demo 入力](npm/core/demo/cases)を、現在のソースツリー（パッケージバージョン `0.0.1-alpha.7`）で測定した結果です。測定方法と再現手順は [BENCHMARK.md](BENCHMARK.md) を参照してください：
...
| **合計** | **79,280 B** | **22,588 B** | **71.5%** | **17,006** | **非可逆 9/9 件を復元** |
