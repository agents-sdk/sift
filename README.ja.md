...
### **コンテキストを 62.3% 削減。推定 14,129 トークンを節約。非可逆ベンチマークはすべて復元成功。**

9 つの内蔵ベンチマーク全体で **75,546 B から 28,447 B** へ削減し、**非可逆 6/6 ケースで原文の復元に成功**しました。[測定結果の詳細を見る。](#どれくらい削減できるか)
...
- **コンテキストコストの削減** — 内蔵ベンチマークでは 75,546 バイトから 28,447 バイトへ、**62.3% 削減**しました。
...
リポジトリ内の固定された 9 つの [demo 入力](npm/core/demo/cases)を、公開済みパッケージ `0.0.1-alpha.7` で測定した結果です。測定方法と再現手順は [BENCHMARK.md](BENCHMARK.md) を参照してください：
...
| 繰り返しプレーンテキスト | 2,723 B | 614 B | 77.5% | 632 | PASS |
| 固有情報と機密値の保護 | 3,125 B | 3,125 B | 0% | 0 | 変更なし |
| **合計** | **75,546 B** | **28,447 B** | **62.3%** | **14,129** | **非可逆 6/6 件を復元** |

これは公開 fixture の透明な測定結果であり、すべてのワークロードに対する保証ではありません。変更なしの 1 件も意図的に掲載しています。保守的なプレーンテキスト処理では固有の事実を削除しません。`tokensSaved` は sift 内蔵の推定値です。実際のトークン数と削減量はモデルの tokenizer と入力データによって変わります。
