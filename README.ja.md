...
### **コンテキストを 73.1% 削減。推定 16,795 トークンを節約。非可逆ベンチマークはすべて復元成功。**

10 個の内蔵ベンチマーク全体で **76,582 B から 20,594 B** へ削減し、**非可逆 8/8 ケースで原文の復元に成功**しました。[測定結果の詳細を見る。](#どれくらい削減できるか)
...
- **コンテキストコストの削減** — 内蔵ベンチマークでは 76,582 バイトから 20,594 バイトへ、**73.1% 削減**しました。
...
リポジトリ内の固定された 10 個の [demo 入力](npm/core/demo/cases)を、現在のソースツリー（パッケージバージョン `0.0.1-alpha.7`）で測定した結果です。測定方法と再現手順は [BENCHMARK.md](BENCHMARK.md) を参照してください：
...
| **合計** | **76,582 B** | **20,594 B** | **73.1%** | **16,795** | **非可逆 8/8 件を復元** |
